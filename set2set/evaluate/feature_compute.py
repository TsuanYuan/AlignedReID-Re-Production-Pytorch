...
def load_experts(experts_file, device_id):
    ...
    with open(experts_file, 'r') as fp:
        for line in fp:
            ...
            model = load_model.AppearanceModelForward(model_path, single_device=device_id)
            models.append(model)
            exts.append(ext)
    return models, exts
    ...
    for i, crop_file in enumerate(crop_files):
        ...
        if os.path.isfile(descriptor_file) and (not force_compute):
            ...
            if not skip_reading:
                ...
                im = misc.crop_pad_fixed_aspect_ratio(im)
                im = cv2.resize(im, (128, 256))
                ims.append(im)
                files_from_gpus.append(crop_file)

        if len(ims) > 0 and (len(ims) == batch_max or i == len(crop_files)-1):
            if load_keypoints and (model.get_model_type() == load_model.Model_Types.HEAD_POSE or model.get_model_type() == load_model.Model_Types.LIMB_POSE or model.get_model_type() == load_model.Model_Types.HEAD_ONLY) :
