...
class ReidTransform(object):
  ...
  @staticmethod
  def crop_pad_fixed_aspect_ratio(im, desired_size=(256, 128), head_top=True):
    ...
    #debug
    #import scipy.misc
    #img_idx = random.randint(1, 100000)
    #scipy.misc.imsave(str(img_idx) + '.jpg', new_im)
    return new_im
  ...
  def pre_process_im(self, im, desired_size=(256, 128), stretch=False):
    ...
    #import scipy.misc
    #img_idx = random.randint(1, 100000)
    #print('writing to ' + str(img_idx) + '.jpg')
    #scipy.misc.imsave(str(img_idx) + '.jpg', im)

    # The original image has dims 'HWC', transform it to 'CHW'.
    #if self.batch_dims == 'NCHW':
    #  im = im.transpose(2, 0, 1)
    im = functional.to_tensor(im)
    im = im.float()
    return im
